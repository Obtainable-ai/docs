...
For the full chat workflow, see [Chatting with the Peer Agent](/guides/ai-assistant).
...
Beyond answering, the Peer Agent can add to the workspace on behalf of members — capturing a thought, drafting a note, or saving a source. **Which of these it's allowed to do is set per workspace** by an admin, in [Workspace settings → Agents](/configuration/workspace-settings#agents), where the agent can also be turned off entirely for that workspace.

## Your control

Each member decides what the Peer Agent may do with *their* content, in [Preferences → AI](/configuration/user-preferences#ai):
