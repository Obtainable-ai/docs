...
<CardGroup cols={2}>
  ...
  <Card title="Web pages" icon="globe">
    Clip web pages from your browser with the [PeerNotes extension](/guides/web-clipper). The page's content and URL are saved together.
  </Card>
...
Sources are private by default, but — like thoughts and notes — each one has a **Share with workspace** toggle. Share a source and the whole team can see it in the **Shared** view of the Sources feed and comment on it. Independently of that, when a shared note references a source, members reading the note can open it for context. Set a default for new sources in [Preferences → Sharing](/configuration/user-preferences#sharing).
